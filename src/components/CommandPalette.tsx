...
export function CommandPalette({ onClose }: { onClose: () => void }) {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    const commands: Item[] = [
      ...
      { id: "go:people", label: "Go to People table", group: "Go to", run: go(() => setTab("people")), hint: "G then P" },
    ...
    >
      <SysModal className="max-w-xl">
        <Dialog aria-label="Command palette" className="flex max-h-[inherit] flex-col">
          <div className="flex shrink-0 items-center gap-2.5 border-b border-stone-200 px-4 py-3 dark:border-stone-800">
            ...
            />
            <kbd className="hidden shrink-0 rounded bg-stone-100 px-1.5 py-0.5 font-mono text-[10px] text-stone-500 sm:inline dark:bg-stone-800 dark:text-stone-400">
              Esc
            </kbd>
          </div>
          ...
          >
            ...
            {results.map((item, i) => {
              ...
              return (
                <div key={item.id}>
                  {first && (
                    <div className="px-4 pt-2 pb-1 text-[10px] font-semibold tracking-widest text-stone-400 uppercase dark:text-stone-500">
                      {item.group}
                    </div>
                  )}
                  ...
                  >
                    ...
                    {item.chord && (
                      <kbd className="shrink-0 rounded bg-stone-100 px-1.5 py-0.5 font-mono text-[10px] text-stone-500 dark:bg-stone-800 dark:text-stone-400">
                        {item.chord}
                      </kbd>
                    )}
                  </div>
          ...
          <div className="pad-safe-bottom flex shrink-0 items-center gap-3 border-t border-stone-200 px-4 py-2 text-[11px] text-stone-500 [--pad-safe-bottom:0.5rem] dark:border-stone-800 dark:text-stone-400">
            <span>

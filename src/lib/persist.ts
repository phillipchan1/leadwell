/**
 * Client-injectable persistence for LeadWell.
 *
 * The browser store and the MCP server share this module so they stay on one
 * mapping. Pass the browser anon client from `repo.ts`, or a service-role
 * client from the MCP — this file never imports Vite env.
 *
 * Sync is upsert-current + tombstone-only delete: a row is removed only when
 * it was in the last baseline and is gone from the current document. Rows
 * another writer (the MCP) inserted while the app was open are left alone.
 */
...
/**
 * Which existing keys should be deleted.
 *
 * - `knownIds` set: only keys the caller already knew about (the last
 *   baseline). Keys that appeared from another writer are left alone.
 * - `knownIds` omitted: full replace — anything not in `keep` goes
 *   (seed / first import).
 */
export function idsToDelete(
  ...
  return existing.filter((k) => {
    if (!k || kept.has(k)) return false;
    return known ? known.has(k) : true;
  });
}

/**
 * Persist an id-keyed collection: upsert every current row, then delete
 * tombstones. Pass `knownIds` from the last baseline so a concurrent MCP
 * insert is not wiped. Omit it for a full replace (writeAll).
 */
export async function pushRows(
  ...
  const keep = rows.map((r) => r[keyField] as string);
  const toDelete = idsToDelete(
    ((existing ?? []) as unknown as Row[]).map((r) => r[keyField] as string),
    keep,
    knownIds
  );
...
export type CollKey = keyof typeof map;
export const COLLECTIONS = Object.keys(map) as CollKey[];
export { map };
...
export async function writeAll(
  ...
  const { error } = await client.from("profiles").upsert(meToRow(userId, d.me));
  ...
  await Promise.all([
    ...COLLECTIONS.map((k) => {
      ...
      return pushRows(client, m.table, "id", userId, rows);
    }),
    pushRows(client, "chats", "chat_key", userId, chatRows(userId, d.chats)),
    pushRows(
      client,
      "node_positions",
      "node_id",
 ...
 */
function collectionIds(items: { id: string }[] | undefined): string[] {
  ...
  for (const k of COLLECTIONS) {
    ...
    jobs.push(
      pushRows(
        ...
        userId,
        rows,
        base ? collectionIds(base[k] as { id: string }[]) : undefined
      )
    );
  }
  ...
  if (!base || base.chats !== d.chats) {
    jobs.push(
      pushRows(
        ...
        "chat_key",
        userId,
        chatRows(userId, d.chats),
        base ? Object.keys(base.chats) : undefined
      )
    );
  }
  if (!base || base.nodePositions !== d.nodePositions) {
    jobs.push(
      pushRows(
        ...
        userId,
        posRows(userId, d.nodePositions),
        base ? Object.keys(base.nodePositions) : undefined
      )
    );
  }
...
/** Upsert specific rows without deleting anything else. MCP writes use this. */
export async function upsertMapped<K extends CollKey>(
  client: SupabaseClient,
  userId: string,
  key: K,
  items: PersistedData[K] extends unknown[] ? PersistedData[K] : never
): Promise<void> {
  const m = map[key];
  const rows = (items as unknown[]).map((item) =>
    (m.toRow as (u: string, item: unknown) => Row)(userId, item)
  );
  if (!rows.length) return;
  const { error } = await client.from(m.table).upsert(rows);
  if (error) throw new Error(`${m.table} upsert: ${error.message}`);
}

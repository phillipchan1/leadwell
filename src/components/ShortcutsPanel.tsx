...
export function ShortcutsPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="grid gap-x-8 gap-y-6 @2xl:grid-cols-2">
        {grouped.map(([group, rows]) => (
          <section key={group} className="break-inside-avoid">
            <h3 className="mb-2 text-[11px] font-semibold tracking-widest text-stone-400 uppercase dark:text-stone-500">
              {group}
            </h3>
              ...
              {rows.map((row) => (
                ...
                >
                  ...
                  <dd className="shrink-0">
                    <kbd className="rounded border border-stone-200 bg-stone-50 px-1.5 py-0.5 font-mono text-[11px] whitespace-nowrap text-stone-600 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300">
                      {row.keys}
                    </kbd>
                  </dd>
                </div>
      ...
      <p className="mt-6 border-t border-stone-100 pt-4 text-xs leading-relaxed text-stone-500 dark:border-stone-800 dark:text-stone-400">

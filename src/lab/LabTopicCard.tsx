...
export function LabTopicCard({
  ...
  const shownTags = topic.tagIds
    ...
    .filter((t): t is LabTag => Boolean(t));

  const suggestedMeeting =
    !topic.meetingId && topic.suggestedMeetingId
      ? meetingById.get(topic.suggestedMeetingId)
      : undefined;
  const suggestedTags =
    topic.tagIds.length === 0
      ? (topic.suggestedTagIds ?? [])
          .map((id) => tagById.get(id))
          .filter((t): t is LabTag => Boolean(t))
      : [];
  const hasSuggestion = Boolean(suggestedMeeting) || suggestedTags.length > 0;
  ...
  if (showMeeting && topic.meetingId) {
    meta.push(<span key="mtg">{meetingById.get(topic.meetingId)?.name}</span>);
  }
    ...
    >
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          {/*
            A suggestion is an offer, not an announcement. The line itself is the
            control — click it to take it, ignore it otherwise, and it disappears
            the moment the topic gets a real tag or meeting. No buttons riding on
            top of the card waiting to be noticed.
          */}
          {hasSuggestion && (
            <button
              type="button"
              ...
              className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 rounded text-left text-caption text-quaternary transition hover:text-teal-700 dark:hover:text-teal-400"
            >
              <span className="opacity-70">suggests</span>
              {suggestedMeeting && (
                <span className="italic opacity-80">{suggestedMeeting.name}</span>
              )}
              {suggestedTags.map((tag) => (
                <TagChip key={tag.id} tag={tag} ghost />
              ))}
            </button>
          )}

 ...
 *
 * The shared `useCardDrag` answers "which column is the pointer over" and
 * nothing else, which was fine for a board whose columns were unordered piles.
 * The planner needs more: a week column is now a stack of template sections,
 * and the order of cards inside a section is the running order of the meeting.
 * So a drop has to resolve to a *zone plus an index*.
 *
